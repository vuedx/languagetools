...
describe('getCompletionsAtPosition', () => {
  ...
  test('should complete object properties (CompletionInfo)', async () => {
    const server = createLanguageServerForTest()
    server.openFile('fixture-completions-import.vue')
    server.sendCommand(Proto.CommandTypes.CompletionInfo, <Proto.CompletionsRequestArgs>{
      file: server.canonicalFileName('fixture-completions-import.vue'),
      line: 2,
      offset: 27
    })
    ...
    expect(server.responses).toHaveLength(1)
    const response: Proto.CompletionInfoResponse = server.responses[0]
    expect(response.command).toBe(Proto.CommandTypes.CompletionInfo)
    expect(response.body).toBeTruthy()
    expect(response.body!.isMemberCompletion).toBe(true)
    expect(response.body!.isGlobalCompletion).toBe(false)

    const names = response.body!.entries.map(entry => entry.name)
    expect(names).toContain('ref')
    expect(names).toContain('computed')
    // already imported.
    expect(names).not.toContain('defineComponent')
  ...
  test('should report suggestions in a vue file', async () => {
    ...
    expect(response.body!.map(item => item.name)).toEqual(['foo'])
  })
  
  test('should report suggestions in a vue file (CompletionInfo)', async () => {
    const server = createLanguageServerForTest()
    server.openFile('fixture-completion-at-position.vue')
    server.sendCommand(Proto.CommandTypes.CompletionInfo, <Proto.CompletionsRequestArgs>{
      file: server.canonicalFileName('fixture-completion-at-position.vue'),
    ...
    expect(response.command).toBe(Proto.CommandTypes.CompletionInfo)
    expect(response.body).toBeTruthy()
    expect(response.body!.entries.map(item => item.name)).toEqual(['foo'])
  })
})
