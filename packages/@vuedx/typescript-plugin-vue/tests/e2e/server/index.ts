...
export class TestVueServer {
  ...
  constructor(
    ...
    options: Partial<TestVueServerOptions>
  ) {
    ...
    Readline.createInterface({ input: this.server.stdout! }).on(
      'line',
      line => {
        if (line[0] !== '{') return
  ...
  public openFile(
    file: string,
    fileContent?: string,
    scriptKindName?: Proto.OpenRequestArgs['scriptKindName']
  ) {
