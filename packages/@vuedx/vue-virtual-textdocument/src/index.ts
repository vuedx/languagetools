import { CompilerError, parse as parseSFC, SFCBlock, SFCDescriptor } from '@vuedx/compiler-sfc';
import { parse as parseQueryString, stringify as stringifyQueryString } from 'querystring';
import { TextDocument, TextDocumentContentChangeEvent, Position, Range } from 'vscode-languageserver-textdocument';
...
export class VirtualTextDocument implements TextDocument {
  ...
  static update(document: VirtualTextDocument, changes: TextDocumentContentChangeEvent[], version: number) {
...
export class VueTextDocument implements TextDocument {
  ...
  public constructor(
    private internal: TextDocument,
    private readonly mode: 'javascript' | 'typescript' = 'typescript'
  ) {
    this.parse();
  }
  ...
  public blockAt(positionOrOffset: Position | number) {
    const offset = typeof positionOrOffset === 'number' ? positionOrOffset : this.offsetAt(positionOrOffset);
  ...
  private parse() {
    ...
    if (descriptor.script) {
      if (descriptor.script.src) {
        const moduleName = JSON.stringify(descriptor.script.src.replace(/\.(j|t)sx?$/, ''));
        this.setBlockDocument(
          descriptor.script,
          this.createBlockDocument({
            ...descriptor.script,
            src: undefined,
            content: `import component from ${moduleName}\nexport * from ${moduleName}\nexport default component`,
          })
        );
      } else {
      ...
      // TODO: Support allowJS and TSonly.
      this.setBlockDocument(
        'script',
        this.createBlockDocument({
          type: 'script',
  ...
  public getBlockDocument(block?: SFCBlock | 'script' | 'template' | BlockSelector | string) {
    if (!block) return;
    ...
    if ('index' in selector && !/^(script|template)$/.test(selector.type) && !this.documents[id]) {
  ...
  private setBlockDocument(block: SFCBlock | 'script' | 'template', document: VirtualTextDocument | null) {
  ...
  private getUriFor(block: SFCBlock) {
    ...
    const uri = URI.file(
      this.uriObject.path +
        virtualFileNameSep +
        ('index' in selector ? `${selector.type}__${selector.index}` : selector.type) +
        '.' +
  ...
  private getLanguageFor(block: SFCBlock): string {
    return block.lang || BlockLanguageMap[block.type as keyof typeof BlockLanguageMap] || 'txt';
  }
  ...
  static create(
    ...
    version: number,
    content: string,
    mode: 'javascript' | 'typescript' = 'typescript'
  ) {
