...
export function compile(source: string, options: Options) {
  ...
  const renderFn: B.FunctionDeclaration = (renderAST.program.body.find((statement) =>
    B.isExportNamedDeclaration(statement)
  ) as B.ExportNamedDeclaration).declaration;
  const generated = (renderFn.body.body.find((statement) => B.isReturnStatement(statement)) as B.ReturnStatement)
    .argument!;
...
function match(html: T.Node, parent: T.ElementNode, js: B.Node) {
  if (B.isCallExpression(js) && B.isIdentifier(js.callee) && js.callee.name === '_h') {
    if (js.arguments.length === 2) {
      assert((js.arguments[0] as B.Identifier).name === '_Fragment', `unexpected argument to _h(): ${js.arguments[0]}`);
      ...
      if (B.isCallExpression(items)) {
        ...
        const nextJs = ((iterator.body as B.BlockStatement).body[0] as B.ReturnStatement).argument as B.CallExpression;

        if (TT.isTemplateNode(html)) {
          if (html.children.length === 1) {
            match((html.children[0] as unknown) as T.Node, (html as unknown) as T.ElementNode, nextJs);
          } else {
    ...
    const vIf = T.findDir(html as T.ElementNode, /^(if|else-if)$/) as T.DirectiveNode;
    // @ts-ignore
...
function matchChildren(parent: ElementNode, children: B.ArrayExpression | B.ObjectExpression) {
  if (B.isArrayExpression(children)) {
    ...
    if (children.properties.length === 1 && children.properties.some((property) => B.isObjectProperty(property))) {
      const { value } = children.properties[0] as B.ObjectProperty;

      assert(B.isCallExpression(value));

      const vSlot = T.findDir(parent, 'slot');
      const slot = (value as B.CallExpression).arguments[0] as B.ArrowFunctionExpression;
      ...
      children.properties.forEach((property, index) => {
        if (B.isObjectProperty(property)) {
          ...
          const vSlot = T.findDir(parent.children[index]! as T.ElementNode, 'slot');
          const slot = (value as B.CallExpression).arguments[0] as B.ArrowFunctionExpression;
          const slotChildren = slot.body as B.ArrayExpression;
...
function findJSNodes(node?: T.Node): T.Node[] {
  ...
  if (TT.isElementNode(node)) {
    node.props.forEach((node) => result.push(...findJSNodes((node as unknown) as T.Node)));
    ...
    result.push(...findJSNodes((node.content as unknown) as T.SimpleExpressionNode));
  }

  if ('children' in node) {
    (node as T.PlainElementNode).children.forEach((node) => result.push(...findJSNodes(node)));
  }
