...
export class OpenVirtualFileCommand extends Installable {
  ...
  public install() {
    ...
    return vscode.commands.registerTextEditorCommand('vue.openVirtualFile', this.onExecute.bind(this));
  }
  ...
  private async onExecute() {
    ...
    if (!editor) {
      return vscode.window.showInformationMessage('There is no active Vue document.');
    }
    ...
    let document = block?.type === 'template' ? container.getDocument('_render') : container.documentAt(position);

    if (!document) {
      document = container.getDocument('_internal')!;
