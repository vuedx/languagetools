...
export class GenerateGrammarCommand extends Installable {
  ...
  public constructor(
    ...
    this.supported = require(Path.resolve(this.context.extensionPath, 'scripts', 'supported.json'));
  }
  ...
  public install() {
    ...
    return vscode.Disposable.from(
      vscode.commands.registerTextEditorCommand('vue.generateGrammar', this.onExecute.bind(this)),
  ...
  private async generate(
    ...
    try {
      ...
      require(Path.resolve(this.rootDir, 'scripts', 'generate-grammar.js')).generate();

      if (changes.length) {
        const ans = await vscode.window.showInformationMessage(
          'New block types found in vue files: \n' +
            changes.map((change) => `<${change.block} lang="${change.language}">`).join(', ') +
