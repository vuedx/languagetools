...
import { openFile, wait, onExtensionReady, getExtensionPath } from '../test';
import { unlinkSync, existsSync } from 'fs';
import { resolve } from 'path';
import { VueExtensionSettings } from '../services/configuration';
...
suite('New language block detection', () => {
  ...
  test('it detects new language block when .vue file is opened', async () => {
    ...
    const config = vscode.workspace.getConfiguration('vue').get<VueExtensionSettings['blocks']>('blocks')!;
