...
function main(version, publish, filter) {
  if (publish && !process.env.VSCE_TOKEN) {
    throw new Error('Set VSCE_TOKEN environment variable with personal access token.');
  }
  ...
  extensions.forEach((name) => {
    ...
    try {
      ...
      if (publish) run(`vsce publish -m 'release(${name}): version %s' -p ${PAT} ${version}`, extensionDir);
...
function getWorkspaceVersion(name) {
  return require(name.endsWith('package.json') ? name : Path.resolve(packagesDir, name, 'package.json')).version;
}
